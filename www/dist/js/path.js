            ...
            if (tell) {
                ...
                for (var i = 0; i < finalres.length; i++) {
                    ...
                    for (var j = 0; j < finalres[i].length; j++) {
                        fr_temp.push(new google.maps.LatLng(finalres[i][j].A, finalres[i][j].F));
                    }
                    finalres[i] = fr_temp;
                }
...
function nextStep(pathJSON, PathLength) {
    ...
    totalL = PathLength;
    //乘客路徑JSON
    passenger_json = file.substring(0, file.length - 1) + ',"path":' + pathJSON + ',"start":' + '{"latitude":"' + (new Number(StartPoint.A)).toFixed(14) + '","longitude":"' + (new Number(StartPoint.F)).toFixed(14) + '"}' + ',"end":' + '{"latitude":"' + (new Number(pathTemp[pathTemp.length - 1].A)).toFixed(14) + '","longitude":"' + (new Number(pathTemp[pathTemp.length - 1].F)).toFixed(14) + '"}}';
